...
void getContours(Mat imgDil, Mat img) {
    ...
    findContours(imgDil, contours, hierachy, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);  //通过预处理的二值图像找到所有轮廓contours
    ...
    for (int i = 0; i < (int)contours.size(); i++)
    {
        ...
        if (area > 1000)    //过滤噪声
        {
            ...
            drawContours(img, conPoly, i, Scalar(255, 0, 255), 2);  //绘制滤除噪音后的所有轮廓
...
int main()
{
    string path = "/home/sunwei/Opencv/形状检测/12.jpg";
    Mat img = imread(path);
